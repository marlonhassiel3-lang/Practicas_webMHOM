...
const app = express();
app.set('trust proxy', 1); // Important for Netlify + Express-Rate-Limit
app.use(express.json());

// Layer 2 Security Middlewares
app.use(cors()); // Allow cross-origin requests securely
app.use(helmet()); // Hide signature and apply 11 HTTP security heads
// Removed mongoSanitize() due to Express 5.0 incompatibility with req.query

const loginLimiter = rateLimit({
  windowMs: 15 * 60 * 1000, // 15 minutes max memory
  max: 10, // Prevent brute-force with max 10 requests per window
  message: { error: 'Demasiados intentos. Por favor, inténtalo de nuevo más tarde.' }
});

// Protect login brute-force attacks
app.use('/api/admin/login', loginLimiter);

// Inicializar conexión sin bloquear el Event Loop (Fix para Mongoose pending promise trap)
connectDB();

app.get('/api/eventos', async (req, res) => {
  try {
    const eventos = await Evento.find().limit(30).lean();
...
app.get('/api/avisos', async (req, res) => {
  try {
    const avisos = await Aviso.find().sort({ fechaPublicacion: -1 }).limit(30);
    res.json(avisos);
  } catch (error) {
...
app.get('/api/solicitudes', async (req, res) => {
  try {
    // Admin solo ve mensajes generales (sin profesor_id)
    const solicitudes = await Solicitud.find({ profesor_id: { $exists: false } }).sort({ fecha: -1 });
...
app.get('/api/solicitudes/profesor/:id', async (req, res) => {
  try {
    // Profesor ve solo los suyos
    const solicitudes = await Solicitud.find({ profesor_id: req.params.id }).sort({ fecha: -1 });
    res.json(solicitudes);
  } catch (error) {
    res.status(500).json({ error: 'Error' });
  }
...
app.put('/api/solicitudes/leidas', async (req, res) => {
  try {
    const filter: any = { estado: 'NUEVO' };
    if (req.body.profesor_id) {
      filter.profesor_id = req.body.profesor_id;
    } else {
      filter.profesor_id = { $exists: false };
    }
    await Solicitud.updateMany(filter, { estado: 'LEIDO' });
    res.json({ success: true });
  } catch (error) {
